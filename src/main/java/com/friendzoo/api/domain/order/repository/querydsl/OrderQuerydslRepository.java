...
import com.friendzoo.api.domain.order.controller.AdminOrderController;
...
public class OrderQuerydslRepository {
    ...
    public Page<Order> findAllBy(AdminOrderController.OrderHisRequestDTO pageRequestDTO) {
        ...
        List<Order> list = queryFactory
                .select(order)
                .from(order)
                .where(getSearchTermContains(pageRequestDTO.getSearchTerm()),
                        getEqOrderDateOfYear(pageRequestDTO.getYear()))
    ...
    private BooleanExpression getSearchTermContains(String searchTerm) {
        if (StringUtils.hasText(searchTerm)) {
            // 상품명 검색
            return order.orderItems.any().product.name.contains(searchTerm.trim()).or(
                    // 주문코드 검색
                    order.code.contains(searchTerm.trim())
            );
        }
        return null;
    }
